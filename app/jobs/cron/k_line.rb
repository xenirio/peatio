      ...
      def self.process
        Market.active.each do |market|
          ::KLineService::AVAILABLE_POINT_PERIODS.each do |period|
            service = ::KLineService[market.ticker, period]
            point = service.get_ohlc(order_by: :desc, limit: 1, offset: false).first
            next if point.blank?

            event_name = service.event_name(period)
            Rails.logger.info { "publishing #{point} #{event_name} stream for #{market.ticker}" }
            ::AMQP::Queue.enqueue_event('public', market.ticker,
                                        event_name, point)
          end
        end
        sleep 2
