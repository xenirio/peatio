...
module Matching
  class Engine
    ...
    def initialize(market, options={})
      @market    = market
      @orderbook = OrderBookManager.new(market.ticker, on_change: method(:publish_increment))
      @initializing = true
      @sequence_number = 1
      @increment_count = 0
    ...
    def publish_snapshot
      @snapshot_time = Time.now
      ::AMQP::Queue.enqueue_event("public", @market.ticker, "ob-snap", {
    ...
    def publish(order, counter_order, trade)
      ...
      Rails.logger.info { "[#{@market.ticker}] new trade - maker_order: #{maker_order.label} taker_order: #{taker_order.label} price: #{price} amount: #{amount} total: #{total}" }
