      ...
      class InternalTransfers < Grape::API
        namespace :internal_transfers do
          ...
          get do
            ...
            present paginate(search.result), with: API::V2::Entities::InternalTransfer, current_user: current_user
          end
          ...
          post do
            receiver = if Member.find_by(uid: params[:username_or_uid]).present?
              Member.find_by(uid: params[:username_or_uid])
            elsif Member.find_by(username: params[:username_or_uid]).present?
              Member.find_by(username: params[:username_or_uid])
            end
