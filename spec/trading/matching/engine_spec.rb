...
describe Matching::Engine do
  ...
  context 'submit market order' do
    ...
    it 'should fill the market order completely' do
      ...
      AMQP::Queue.expects(:enqueue).with(:trade_executor, { action: "execute", trade: { market_id: market.ticker, maker_order_id: ask1.id, taker_order_id: mo.id, strike_price: ask1.price, amount: ask1.volume, total: '1.0'.to_d } }, anything)
      AMQP::Queue.expects(:enqueue).with(:trade_executor, { action: "execute", trade: { market_id: market.ticker, maker_order_id: ask2.id, taker_order_id: mo.id, strike_price: ask2.price, amount: ask2.volume, total: '2.0'.to_d } }, anything)
      AMQP::Queue.expects(:enqueue).with(:trade_executor, { action: "execute", trade: { market_id: market.ticker, maker_order_id: ask3.id, taker_order_id: mo.id, strike_price: ask3.price, amount: '0.4'.to_d, total: '1.2'.to_d } }, anything)
  ...
  context 'submit limit order' do
    context 'fully match incoming order' do
      it 'should execute trade' do
        AMQP::Queue.expects(:enqueue)
                 .with(:trade_executor, { action: "execute", trade: { market_id: market.ticker, maker_order_id: ask.id, taker_order_id: bid.id, strike_price: price, amount: volume, total: '50.0'.to_d } }, anything)
    ...
    context 'partial match incoming order' do
      ...
      it 'should execute trade' do
        AMQP::Queue.expects(:enqueue)
                 .with(:trade_executor, { action: "execute", trade: { market_id: market.ticker, maker_order_id: ask.id, taker_order_id: bid.id, strike_price: price, amount: 3.to_d, total: '30.0'.to_d } }, anything)
    ...
    context 'fully match order after some cancellatons' do
      ...
      it 'should match bid with high ask' do
        ...
        AMQP::Queue.expects(:enqueue)
                 .with(:trade_executor, { action: "execute", trade: { market_id: market.ticker, maker_order_id: high_ask.id, taker_order_id: bid.id, strike_price: high_ask.price, amount: high_ask.volume, total: '30.0'.to_d } }, anything)
        subject.submit(bid)
  ...
  context 'dryrun' do
    ...
    it 'should not publish matched trades' do
      ...
      expect(subject.queue.first).to eq [:trade_executor, { action: "execute", trade: { market_id: market.ticker, maker_order_id: ask.id, taker_order_id: bid.id, strike_price: price, amount: volume, total: '50.0'.to_d } }, { persistent: false }]
    end
  end
  ...
  context 'publish_increment' do
    ...
    it 'should publish increment of orderbook' do
      ::AMQP::Queue.expects(:enqueue_event).with("public", market.ticker, "ob-inc", { "asks" => ["10.0", "5.0"], "sequence" => 2, })
      ::AMQP::Queue.expects(:enqueue_event).with("public", market.ticker, "ob-inc", { "bids" => ["10.0", "5.0"], "sequence" => 3, })

      subject.publish_increment(market.ticker, :ask, ask.price, ask.volume)
      subject.publish_increment(market.ticker, :bid, bid.price, bid.volume)
    end
  end
  ...
  context 'publish_snapshot' do
    ...
    it 'should publish snapshot of orderbook' do
      ...
      ::AMQP::Queue.expects(:enqueue_event).with("public", market.ticker, "ob-snap", {
    ...
    context 'periodic publish snapshot time' do
      ...
      it 'should publish snapshot of orderbook and set increment_count to 1' do
        ...
        ::AMQP::Queue.expects(:enqueue_event).with("public", market.ticker, "ob-inc", { "asks" => ["11.0", "2.0"], "sequence" => 2 })
        subject.publish_increment(market.ticker, :ask, bid1.price, bid1.volume)
