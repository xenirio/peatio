...
describe API::V2::Admin::InternalTransfers, type: :request do
  ...
  let(:level_3_member) { create(:member, :level_3, uid: 'ID84BF61C8H0') }
  ...
  describe 'GET /api/v2/admin/internal_transfer' do
    ...
    context 'filtering' do
      ...
      it 'returns orders for specific sender by uid' do
        api_get endpoint, params: { sender_uid: admin.id }, token: token
        result = JSON.parse(response.body)
        
        expect(response).to be_successful
        expect(result.map{|r| r['sender']}.size).to eq 4
        expect(result.map{|r| r['sender']}).to all eq admin.id
      end

      it 'returns orders for specific receiver by uid' do
        api_get endpoint, params: { receiver_uid: level_3_member.uid }, token: token
        result = JSON.parse(response.body)
  
        expect(response).to be_successful
        expect(result.map{|r| r['receiver']}.size).to eq 4
        expect(result.map{|r| r['receiver']}).to all eq level_3_member.id
      end
    end
  end
end
